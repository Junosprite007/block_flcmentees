...
class block_flcmentees extends block_base {

  function init() {
    $this->title = get_string('pluginname', 'block_flcmentees');
  }
  ...
  function specialization() {
    $this->title = isset($this->config->title) ? $this->config->title : get_string('newmenteesblock', 'block_flcmentees');
  }
  ...
  function get_content() {
    ...
    $allusernames = $userfieldsapi->get_sql('u', false, '', '', false)->selects;
        if ($usercontexts = $DB->get_records_sql("SELECT c.instanceid, $allusernames
                                                    ...
                                                    GROUP BY c.instanceid, $allusernames
   ...
   * The FLC mentees block can only be docked if it has a non-empty title.
   * @return bool
   */
